...
<template>
  <div
    ...
    id="popup"
    class="z-30 bg-app-fondo w-screen h-screen fixed pt-10 top-0 left-0"
  >
    ...
    <div class="">
      <div class="flex flex-wrap">
        <CardData
          v-for="value in listPaginationSelect"
          :key="value.name"
          class="m-1"
        >
          <template v-slot:img>
      ...
      <PaginationNav
        :idPageList="NavBtnSelect"
        @changeListPage="pagePagination"
        :countPage="listCharacterPagination"
      >
        <template v-slot:PaginationBtn>
          <PaginationBtn
            :class="key == NavBtnSelect ? '!bg-sky-400' : ''"
            v-for="(value, key) in listCharacterPagination"
            @click="pagePagination(key)"
            :key="value.name"
          >
            {{ key + 1 }}
          </PaginationBtn>
